...
# ─────────────── GitHub 圖片路徑 ───────────────
GITHUB_BASE = "https://raw.githubusercontent.com/homuch/food-classify-game/main/食物圖/"

def get_image_url(card_name: str) -> str:
    """將卡片名稱對應到 GitHub 圖片 URL（特殊卡去掉★）"""
    base = card_name.lstrip("★").strip()
    from urllib.parse import quote
    return GITHUB_BASE + quote(base) + ".jpg"
        ...
        for row_start in range(0, len(rem_cards), COLS_PER_ROW):
            ...
            for col_idx, name in enumerate(row_cards):
                with cols[col_idx]:
                    ...
                    st.image(get_image_url(name), use_container_width=True)
                    ...
                    for row_start in range(0, len(placed), IMG_COLS):
                        ...
                        for ci2, pname in enumerate(row):
                            with img_cols[ci2]:
                                ...
                                st.image(get_image_url(pname), use_container_width=True)
